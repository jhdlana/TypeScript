...
let button10 = document.querySelector<HTMLButtonElement>("#myButton1");
...
// 4. 인터페이스 generic
// 객체 - { key(키) : value(값) }인데 value에 타입을 지정해줌! (원래 보통 key는 문자열로 사용함 name, age.. 등등 이 다 문자열 타입이므로 항상 string으로 지정)
// 타입을 맵핑해중 -> string이면 string, number이면 number
